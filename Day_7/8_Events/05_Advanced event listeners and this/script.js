...
//3_callback function
const toggleLid = function(event, incomingButton, incomingData){
  console.log("And..: ", incomingData);
  console.log("Event..: ", event);
  let findBackpack = backPackData.find(({id}) => id === incomingButton.parentElement.id);
  findBackpack.lidOpen === true ? findBackpack.lidOpen = false: findBackpack.lidOpen = true;


  //Button text
  incomingButton.innerText === "Open lid" ?  incomingButton.innerText = "Close lid": incomingButton.innerText = "Open lid";

  //Lid open text
  let lidStatus = incomingButton.parentElement.querySelector(".backpack__lid span");
...
let data = "Arg that I want to pass to toggleLid function."


btnLid.addEventListener('click', (event) => {
  toggleLid(event, btnLid, data);
});
